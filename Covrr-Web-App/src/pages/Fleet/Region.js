...
const Region = ({ regions, onRegionSelect, onDepotSelect, selectedRegion }) => {
      ...
      <div key="region" className="animate-slideIn">
        ...
        {isLoading ? (
          ...
          regions.map((region, index) => (
            <div key={index}
              className={`bg-cardBackground hover:ring-1 hover:ring-primary rounded-lg shadow mb-4 cursor-pointer transition-all duration-300 ${expandedIndex === index
                ? "max-h-96 outline outline-1 outline-primary"
                : "max-h-[100px] overflow-hidden"
                }`}
            >
              <div onClick={() => handleCardClick(index)} className="p-4 pb-0 ">
                <div className="flex items-center mb-2">
                  <img src={require("../../assets/images/icons/fleet_lcon.svg").default}
                    alt="Region" className="w-7 h-7 mr-2" />
                ...
                <div className="flex w-full gap-x-3 justify-between text-sm mt-2">
                  <div className={`p-2 flex justify-between rounded-md w-full ${expandedIndex === index ? "bg-black" : "bg-muted2 "}`}>
                    <span className={`text-sm whitespace-nowrap ${expandedIndex === index ? "text-white/80" : "text-text4"}`}>
                      No of Depots
                    </span>
                    <span className={`font-bold ${expandedIndex === index ? "text-white" : "text-text2"}`}>
                      {region.depots.length}
                    </span>
                  </div>
                  <div className={`p-2 flex justify-between rounded-md w-full ${expandedIndex === index ? "bg-black" : "bg-muted2 "}`}>
                    <span className={`text-sm whitespace-nowrap ${expandedIndex === index ? "text-white/80" : "text-text4"}`}>
                ...
                <div className="mt-4 bg-cardBackground " key={index}>
                  <div className="flex flex-col gap-y-2">
                    {region.depots.map((depot, depotIndex) => (
                      ...
                      >
                        <h4 className="text-text3 font-semibold text-sm tabular-nums">
                          {depot.name}
                        </h4>
                        <div className="flex gap-x-6 items-end justify-end">
                          <div className="flex items-center gap-x-1 w-10">
                            ...
                            <span className="text-text1 text-sm font-medium tabular-nums ">
                              {depot.fleets}
                            </span>
                          </div>
                          <div className="flex items-center gap-x-1 w-10">
                            <DriverIcon className="size-5 text-text5 opacity-70 fill-none" />
                            <span className="text-text1q text-sm font-medium tabular-nums mt-[-1px]">
                              {depot.drivers}
                            </span>
                          </div>
