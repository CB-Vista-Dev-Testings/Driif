...
const VideoPlayer = ({ url, title }) => {
    ...
    return (
        <div className='border-[6px] border-border shadow-xl min-w-[200px] rounded-lg overflow-hidden pb-4'>
            <div data-vjs-player>
                <video
                    ref={videoRef}
                    className="video-js vjs-default-skin vjs-big-play-centered !h-[115%]"
                />
            </div>
        </div>
    );
...
const FleetVideos = () => {
    ...
    return (
        <div className='relative'>
            <ArrowLeftCircleIcon
                onClick={scrollLeft}
                className="absolute top-1/2 -left-4 bg-primary dark:bg-border transform -translate-y-1/2 w-10 h-10 rounded-full text-white dark:text-text5 cursor-pointer z-10 hover:opacity-80"
            />
            <ArrowRightCircleIcon
                onClick={scrollRight}
                className="absolute top-1/2 -right-4 bg-primary dark:bg-border transform -translate-y-1/2 w-10 h-10 rounded-full text-white dark:text-text5 cursor-pointer z-10 hover:opacity-80"
            />
            <div
                ref={containerRef}
                className="flex flex-row overflow-x-auto theme-scrollbar gap-x-4 p-4 px-0 relative scroll-smooth"
                onWheel={handleWheel}
            >
